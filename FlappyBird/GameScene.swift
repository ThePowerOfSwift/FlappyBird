...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func setupItem() {
        //アイテムの画像を読み込む
        let itemTexture = SKTexture(imageNamed: "leaf")
        ...
        let createItemAnimation = SKAction.run({
            ...
            //アイテムを作成
            let item_leaf = SKSpriteNode(texture: itemTexture)
            item_leaf.position = CGPoint(x: 0.0, y: item_y)
            item.addChild(item_leaf)
